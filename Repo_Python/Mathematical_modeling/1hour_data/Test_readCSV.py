...
for j in range(24):
	...
	plt.figure(j+1,figsize=(16,6.5))

	plt.scatter(x_list, y_list, s=0.1, c=v_list, alpha=0.8)
	if x_list == []:
		continue
	plt.xlim(min(x_list),max(x_list))
	plt.ylim(min(y_list),max(y_list))

	plt.scatter(xlow_list, ylow_list, s=0.5, c=vlow_list, alpha=1.0)
	plt.xlim(min(x_list),max(x_list))
	plt.ylim(min(y_list),max(y_list))
