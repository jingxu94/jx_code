				...
				while choose_goods_flag:
					...
					if goods_id == "q" or goods_id == "Q":
						...
						if result:
							...
							nextflag = False
							while not nextflag:
								donext = input("继续购物(y)，返回上一级(q)：").strip().lower()
								if donext == "y" or donext == "Y":
									break
								elif donext == "q" or donext == "Q":
									choose_goods_flag = False
									break
								else:
									continue
		...
		if userobj.islogin:
			...
			while not _chooseflag:
				...
				if _choose not in ( "1", "2", "3", "4", "5", "6" ):
					common.show_message("请选择正确的功能编号！", "ERROR")
					continue
				else:
					_chooseflag = True
			if _choose == "6":
				...
				if _choose in ( "1", "2", "5" ):
					moduleobj = getattr( modulepy, "users" )
					classobj = getattr( moduleobj, "Users" )
					func = getattr( classobj, func_dict[_choose]["func"] )
				else:
					moduleobj = getattr( modulepy, "report" )
					func = getattr( classobj, func_dict[_choose]["func"] )
				func(userobj)
		else:
			userobj.login()
